...
| 多选 | demo/multiple.md |
| 选择确认 | demo/needConfirm.md |
| 切换不同的选择器 | demo/switchable.md |
| 日期格式 | demo/format.md |
| 值格式化 | demo/value-format.md |
| 日期时间选择 | demo/time.md |
| 格式对齐 | demo/mask.md |
| 日期限定范围 | demo/date-range.md |
| 禁用 | demo/disabled.md |
| 不可选择日期和时间 | demo/disabled-date.md |
| 允许留空 | demo/allow-empty.md |
| 选择不超过一定的范围 | demo/select-in-range.md |
| 预设范围 | demo/preset-ranges.md |
| 额外的页脚 | demo/extra-footer.md |
| 三种大小 | demo/size.md |
| 定制单元格 | demo/cell-render.md |
| 定制面板 | demo/components.md |
| 外部使用面板 | demo/external-panel.md |
| 佛历格式 | demo/buddhist-era.md |
...
### 国际化配置 
默认配置为 en-US，如果你需要设置其他语言，推荐在入口处使用我们提供的国际化组件，详见：[ConfigProvider](../config-provider/docs.md/)。

如有特殊需求（仅修改单一组件的语言），请使用 locale 参数，参考：[默认配置](https://github.com/ant-design/ant-design/blob/masterdocs.md/locale/example.json)。
...
| locale | 国际化配置 | object | [默认配置](https://github.com/ant-design/ant-design/blob/masterdocs.md/locale/example.json) | - |
| minDate | 最小日期，同样会限制面板的切换范围 | dayjs | - | - |
| maxDate | 最大日期，同样会限制面板的切换范围 | dayjs | - | - |
| mode | 日期面板的状态（[设置后无法选择年份/月份？](../../docs/vue/faq.md#when-set-mode-to-datepickerrangepicker-cannot-select-year-or-month-anymore)） | `time` \| `date` \| `month` \| `year` \| `decade` | - | - |
...
| showTime | 增加时间选择功能 | Object \| boolean | [TimePicker Options](../time-picker/docs.md#api) | - |
...
### 当我指定了 DatePicker/RangePicker 的 `mode` 属性后点击后无法选择年份月份？ 
请参考 [FAQ](../../docs/vue/faq.md#when-set-mode-to-datepickerrangepicker-cannot-select-year-or-month-anymore)

### 为什么选完年之后会直接切回日期面板而不是月面板？ 
选完年之后会直接切回日期面板而不是月面板，这是为了减少用户操作步骤。在绝大多数场景下，用户修改年份的同时，很可能不会修改月份，所以在选完年之后，直接切回日期面板更符合用户需求。

### 如何在 DatePicker 中使用自定义日期库（如 dayjs）? 
请参考 [Use custom date library](../../docs/vue/use-custom-date-library.md#datepicker)

### 为什么全局设置了 dayjs.locale 但 DatePicker 语言没变化？ 
DatePicker 在 v4 默认将 `locale` 设置为 `en`，你可以通过 DatePicker 的 `locale` 属性或 [ConfigProvider `locale`](../config-provider/docs.md) 属性来指定语言。

#### 日期相关的组件语言为什么不生效？

请查看 FAQ [Date-related-components-locale-is-not-working?](../../docs/vue/faq.md#date-related-components-locale-is-not-working)

### 如何修改周的起始日？ 
请使用正确的 [language](../../docs/vue/i18n.md) ([#5605](https://github.com/ant-design/ant-design/issues/5605))，或更新 dayjs `locale` 配置：
